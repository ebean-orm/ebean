...
public class TestInheritance extends BaseTestCase {
  ...
  public void testFindCount() {
    ...
    gp1 = server().find(GrandParentPerson.class).where().eq("name", "Franz").findUnique();
    assertEquals("FamilyName A", gp1.getFamilyName());
    assertEquals("Description A", gp1.getBasicSameName().getDescription());
    
    
    gp1 = server().find(GrandParentPerson.class).where().eq("basicSameName.name", "FamilyName A").findUnique();
    assertEquals("FamilyName A", gp1.getFamilyName());
    assertEquals("Description A", gp1.getBasicSameName().getDescription());
    
    
    gp1 = server().find(GrandParentPerson.class).where().eq("basicSameName.description", "Description A").findUnique();
    assertEquals("FamilyName A", gp1.getFamilyName());
    assertEquals("Description A", gp1.getBasicSameName().getDescription());
