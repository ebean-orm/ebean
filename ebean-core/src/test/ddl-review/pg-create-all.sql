...
create index if not exists ix_contact_last_name_first_name on contact (last_name,first_name);
create index if not exists ix_e_basic_name on e_basic (name);
create index if not exists ix_efile2_no_fk_owner_id on efile2_no_fk (owner_id);
create index if not exists ix_ecsm_values_host_id on ecsm_values (host_id);
create index if not exists ix_order_referenced_parent_type on order_referenced_parent (type);
create index if not exists ix_organization_node_kind on organization_node (kind);
create unique index concurrently if not exists ix_t_detail_with_other_namexxxyy_lowername on t_detail_with_other_namexxxyy (lower(name));
...
create index if not exists ano_3 on test_annotation_base_entity (direct);
create index if not exists ano_1 on test_jakarta_annotation_base_entity (direct);
create index if not exists ano_2 on test_jakarta_annotation_base_entity (direct);
...
alter table hembi_bean add column sys_period tstzrange not null default tstzrange(current_timestamp, null);
create table hembi_bean_history(like hembi_bean);
...
create or replace function hembi_bean_history_version() returns trigger as $$
declare
  lowerTs timestamptz;
  upperTs timestamptz;
begin
  ...
  if (TG_OP = 'UPDATE') then
    insert into hembi_bean_history (sys_period,part, brand, name, description, version) values (tstzrange(lowerTs,upperTs), OLD.part, OLD.brand, OLD.name, OLD.description, OLD.version);
...
create trigger hembi_bean_history_upd
  before update or delete on hembi_bean
  for each row execute procedure hembi_bean_history_version();
